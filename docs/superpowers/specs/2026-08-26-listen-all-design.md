...
**Known limitation.** iOS Safari fires no `boundary` events, so `createPlayer`'s reported fraction there is driven by the clock estimate (`CHARS_PER_SECOND = 5.5`) rather than by the synthesizer's true position. On a long list the now-playing label can drift by around a verse. This is the same limitation the existing per-verse progress bar already ships with; the alternative — chaining one player per verse to get an exact index — would give up whole-list seeking and re-introduce the state machine this design removes.
