...
1. **Header synonym.**
2. **Content probe (`cite` only).** The column with the greatest share of cells that `parsePassageRef` accepts, provided that share is at least half. Cell content is stronger evidence than a header word the sheet's author invented; a leader's "순번 / 암송구절 / 확인" sheet resolves correctly this way even when 암송구절 is not in the table above.

   The probe asks twice, because header rule 2 needs the citation column and the citation probe wants the header gone — a circle. **The header-free sample (rows 1 onward) gets first refusal; the whole grid is the fallback.** Sampling header-free matters because one unrecognised label row among a handful drags a column's share below the threshold and costs it the match. Keeping the whole grid as a fallback matters because on a two-row paste whose second reference will not parse, row 0 is the only evidence there is, and dropping it would demote a real citation column and import the titles in its place. Both readings are wrong alone; first-refusal-then-fallback is right for both.
3. **Length probe (`w` and `title`).** Among the columns still free, `w` takes the one with the greatest mean cell length if that mean is at least 20 characters; `title` then takes the first remaining column whose mean is at least 2 and under 20, skipping any column whose sampled cells are all digits. Bodies are long and titles are short, and that alone separates them without a header — but "short" is not enough on its own: a 순번 column of row numbers and a 확인 column of O/X marks are both short, and neither is anything to call a verse.
4. **Claim-breaking probe (`cite` only).** If nothing unclaimed reads as a reference, run the content probe again over *every* column, including ones a header word already claimed, and release that claim. A column headed 본문 whose cells are all references is the citation column; the same "content beats vocabulary" principle as rule 2, applied where vocabulary got there first. Without this, a table headed only 제목 and 본문 leaves `cite` aliased onto one of them and a single column fills two roles.
5. **Positional fallback.** `cite` takes the leftmost unclaimed column, else column 0. `title` and `w` take nothing — an unassigned `w` is not a loss, because the fill supplies it.
...
**Known limits, accepted.** Two columns whose mean length both clear 20 leave the runner-up unmapped, and "longest wins" cannot tell scripture from a longer notes column. A title that reads as a book abbreviation — 시 1 is both Psalms 1 and "poem 1" — can be taken for the citation column by rule 4. A title column of bare numbers is skipped by rule 3's digit guard. Each is a wrong *guess*, shown on the confirm screen and corrected with one dropdown, which is the trade this design makes on purpose: three roles, opinionated heuristics, and a person with the last word.
