...
링크가 못 하는 일이 하나 있습니다 — **어느 사본에 도착할지 고르지 못합니다.**

iOS는 홈 화면에 추가한 웹앱마다 WebKit 저장소 컨테이너를 따로 줍니다. Safari와도,
다른 홈화면 웹앱과도 IndexedDB가 격리됩니다. 그래서 홈 화면에 MemScripture를 둔
독자가 다른 곳에서 이 링크를 따라오면 — Safari 탭이든, 설치된 다른 웹앱이 스코프 밖
주소를 밀어 넣는 인앱 창이든 — **origin도 맞고 DB 이름도 맞는데 컨테이너가 다릅니다.**
저장은 성공하고, 독자가 쓰는 앱에는 아무것도 안 보입니다.

링크로는 못 고칩니다. 조각이든 쿼리스트링이든 리다이렉트든 도착지의 컨테이너를
바꾸지 못하고, iOS의 웹앱은 네이티브 앱처럼 자기 URL을 가로챌 수 없습니다.
Android는 해당 없습니다 — 설치된 PWA가 브라우저와 origin 저장소를 공유해서
링크가 그대로 동작합니다.

넘어가는 길은 **클립보드**뿐입니다(시스템 전역이라 모든 컨테이너를 넘습니다).
그래서 받는 쪽은:

- 조각 없이 `/oyo/import`를 열면 **링크 붙여넣기 화면**을 보여줍니다. 나의 구절의
  가져오기 메뉴에도 같은 입구가 있습니다.
- `readFragmentParam`이 조각뿐 아니라 **주소 전체**를 받습니다 — 클립보드에 담기는
  건 조각이 아니라 주소니까요.
- iOS이면서 홈화면 앱이 아닐 때 **경고와 [링크 복사]** 를 띄웁니다. 막지는 않습니다.
  홈화면 사본이 있는지 앱은 알 수 없고, 없는 독자에겐 그 자리 저장이 정답입니다.

감지는 `src/lib/oyo/handoff.ts`에 있습니다.
...
**장절과 본문은 보내는 쪽의 몫**이라 둘 다 필수이고, 하나라도 빠진 행은
버려집니다 — 장절 없는 구절은 다시 찾을 수 없고, 본문 없는 구절은 외울 것이
없습니다. **제목은 사용자의 몫**이고 **선택**입니다. 보내는 쪽이 제안할 수 있고,
사용자가 고치거나 비워둘 수 있습니다. 비워두면 제목 없이 저장되고, 카드에서는
**장절이 제목 자리에 표시**됩니다 — 장절을 제목으로 *저장*하지는 않습니다.
그러면 "제목 없음"이 "자기 장절로 이름 붙임"이라는 다른 사실로 바뀌고, 동기화
파일과 내보내기까지 따라가서 나중에 구분할 수 없게 됩니다.
...
1. 행마다 **제목란**이 있습니다. `title`을 보냈으면 그 값이 초기값이고, 아니면
   빈 칸(placeholder `제목 (없으면 장절)`)입니다. 비워둔 채 저장해도 됩니다.
