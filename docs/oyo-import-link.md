...
1. 조각을 해독해 목록을 보여줍니다. 이미 같은 장절이 내 구절에 있으면
   **"이미 있음"** 배지를 달고 기본 해제 상태로 둡니다 — 같은 링크를 두 번 눌러도
   쌍둥이가 생기지 않습니다.
1. 행마다 **제목란**이 있습니다. `title`을 보냈으면 그 값이 초기값이고, 안 보냈으면
   비워둔 채 장절을 placeholder로 보여줍니다 — 그래야 사용자가 상자를 먼저 지우지
   않고 바로 타이핑할 수 있습니다. 끝까지 비워두면 장절이 제목이 됩니다.
