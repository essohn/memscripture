			...
			>
				...
				<p class="mt-1 text-[11px] leading-[1.7] text-[var(--color-text-secondary)]">
					{#if deviceVoices.length > 0}
						...
						고르면 그대로 사용합니다.
					{:else}
						기기가 음성을 하나도 알려주지 않았습니다. 안드로이드에서 이 브라우저만 조용하고
						삼성 인터넷에서는 소리가 난다면 <b>음성 엔진</b> 문제입니다: 크롬·웨일·빙은 모두
						같은 엔진을 거치는데, 기기의 기본 엔진이 삼성 TTS로 되어 있으면 그 길이
						막힙니다. <b>안드로이드 설정 → 일반 → 텍스트 음성 변환 출력</b>에서 선호하는
						엔진을 <b>Google 텍스트 음성 변환</b>으로 바꾸고, 한국어 데이터를 내려받으면
						됩니다. 그 앱이 없으면 Play 스토어에서 <b>Speech Recognition &amp; Synthesis
						from Google</b>을 설치하세요. 아이폰은 <b>설정 → 손쉬운 사용 → 라이브 음성</b>
						입니다.
					{/if}
				</p>
			</div>
